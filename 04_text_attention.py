...
def create_text_att():
    ...
    x = Bidirectional(CuDNNLSTM(128, return_sequences=True))(embedding_input)
...
for i, (train_index, valid_index) in enumerate(skf.split(x_train, train['label'])):
    ...
    model = create_text_att()
    model.compile(loss='categorical_crossentropy',
                  optimizer='rmsprop',
                  metrics=['acc'])
    model.summary()
    checkpoint = ModelCheckpoint(filepath='models/att_text_{}.h5'.format(i + 1),
                                 monitor='val_loss',
...
sub['label'] = labels
sub.to_csv('result/att.csv', index=None)
...
train['pred'] = labels
# 分类报告
train.to_excel('result/att_train.xlsx', index=None)
