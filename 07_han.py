...
data = pad_sequences(sequences, maxlen=MAX_SEQUENCE_LENGTH*5)
...
x_train = x_train.reshape((len(x_train), 5, MAX_SEQUENCE_LENGTH))
x_test = x_test.reshape((len(x_test), 5, MAX_SEQUENCE_LENGTH))
...
def create_text_rnn():
    ...
    x_word = embedding_layer(input_word)
    x_word = Bidirectional(CuDNNLSTM(128, return_sequences=True))(x_word)  # LSTM or GRU
    x_word = Attention(MAX_SEQUENCE_LENGTH)(x_word)
    model_word = Model(input_word, x_word)

    # Sentence part
    input = Input(shape=(5, MAX_SEQUENCE_LENGTH))
    ...
    x_sentence = Attention(5)(x_sentence)
...
for i, (train_index, valid_index) in enumerate(skf.split(x_train, train['label'])):
    ...
    model.compile(loss='categorical_crossentropy',
                  ...
                  metrics=['acc'])
    model.summary()
    checkpoint = ModelCheckpoint(filepath='models/han_text_{}.h5'.format(i + 1),
                                 monitor='val_loss',
...
sub['label'] = labels
sub.to_csv('result/han.csv', index=None)
...
train['pred'] = labels
# 分类报告
train.to_excel('result/han_train.xlsx', index=None)
